...
PoseEstimator::PoseEstimator(const char *file_path) {
    char file[200];
    // load a model
    sprintf(file, "%s/images/duck.png", file_path);
    textureImage["duck"] = imread(file);

    sprintf(file, "%s/models/duck.obj", file_path);
    loadObjFile(file, vertexbuffer["duck"], vertices["duck"], uvbuffer["duck"], uvs["duck"], normalbuffer["duck"],
                normals["duck"], elementbuffer["duck"], indices["duck"],
                tbo["duck"], numberOfVertices["duck"], numberOfIndices["duck"]);
    ...
    if (createRenderProgram(shader["color_vertex"], shader["color_fragment"], program["duck"], texture["duck"],
                            textureSampler["duck"], textureImage["duck"], MatrixID["duck"], ViewMatrixID["duck"],
                            ModelMatrixID["duck"], LightPositionID["duck"]) == GL_FALSE)
        return;

    ModelMatrix["duck"] = Matrix4f::Identity();
    ViewMatrix["duck"] = Matrix4f::Identity();
    ViewMatrix["duck"].topRightCorner(3, 1) << 0, -0.2, -0.3;
    ...
    cout << "ModelMatrix\n" << ModelMatrix["duck"] << endl;
    cout << "ViewMatrix\n" << ViewMatrix["duck"] << endl;
...
void PoseEstimator::getPose(string &object, Mat img_camera) {
    VectorXd pose(6), grad(6);
    pose << 0, -0.2, -0.6, 0, 0, 0;
//    double lambda = 0.00001;
    for (uint iter = 0; iter < 1000; iter++) {
        ...
        CUDA_CHECK;
        cudaMalloc(&d_img, WIDTH * HEIGHT * sizeof(uchar));
        CUDA_CHECK;
        // copy data to gpu
        vertices[object][0] = glm::vec3(1,0,0);
        cudaMemcpy(d_vertices, vertices[object].data(), vertices[object].size() * sizeof(float3), cudaMemcpyHostToDevice);
        CUDA_CHECK;
        cudaMemcpy(d_normals, normals[object].data(), normals[object].size() * sizeof(float3), cudaMemcpyHostToDevice);
        CUDA_CHECK;
        // set result image to zero
        cudaMemset(d_img, 0, WIDTH * HEIGHT * sizeof(uchar));
        CUDA_CHECK;
        ...
        costFcn <<<grid,block>>> (d_vertices,d_normals,d_vertices_out,d_normals_out,d_img,vertices[object].size());  CUDA_CHECK;
        ...
        cudaMemcpy(res,d_img,WIDTH*HEIGHT*sizeof(uchar),cudaMemcpyDeviceToHost); CUDA_CHECK;
        cudaMemcpy(vertices_out,d_vertices_out,vertices[object].size()*sizeof(float3),cudaMemcpyDeviceToHost); CUDA_CHECK;
        cudaMemcpy(normals_out,d_normals_out,normals[object].size()*sizeof(float3),cudaMemcpyDeviceToHost); CUDA_CHECK;

//        for(uint i=0;i<vertices[object].size();i++){
//            Vector4f v,w;
//            v<< vertices[object][i][0],vertices[object][i][1],vertices[object][i][2],1;
//            w = cameraPose *  v;
//            cout << K*w<< endl;
...
//        positions_out[idx].x = pixel.x/pixel.z;
//        positions_out[idx].y = pixel.y/pixel.z;
//        positions_out[idx].z = pixel.z/pixel.z;

        int2 pixelCoord;
        pixelCoord.x = (int)pixel.x/pixel.z;
        pixelCoord.y = (int)pixel.y/pixel.z;
        if(pixelCoord.x>=0 && pixelCoord.x<=WIDTH && pixelCoord.y>=0 && pixelCoord.y<=HEIGHT) // if inside image make white pixel
            img[pixelCoord.y*WIDTH+pixelCoord.x] = 255;
    }
}
...
void PoseEstimator::indexVBO(vector<glm::vec3> &in_vertices, vector<glm::vec2> &in_uvs, vector<glm::vec3> &in_normals,
